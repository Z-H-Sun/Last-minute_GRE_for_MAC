def m_explains(mode = 0) # mode = 1 : 只显示例句的模式
  if @ln == 26
    ...
    @wd[1].each do |x|
      ...
      puts unless mode.zero? #将词组中出现的单词标出. 考虑了三单、过去、现在的变形. (将词组中非字母部分替换为通配符. 要考虑末尾y变为i, 末尾双写. 匹配单词边界 (空格)). 最后, 连续的标记间的空格也要一起标出.
      system 'say ' + x[1] if mode.zero? # 朗读解释
    end
    if mode.zero? then print "\n\n" else print ' 请按回车键继续.'; STDIN.gets end
  else
    if mode.zero?
      @wd[1].each do |x|
        eng = x[0].split(']', 2) # 朗读词性
        print "\n \033[1;41;37m#{eng[0]}]\033[0m#{eng[1]}	\033[1;4;7;40;37m#{x[1]}\033[0m\n" # 标出词性、中文释义
        if eng[0].include?('adv') then system 'say 副词' elsif eng[0].include?('v') then system 'say 动词' end
        if eng[0].include?('conj') then system 'say 连词' elsif eng[0].include?('n') then system 'say 名词' end
        system 'say 形容词' if eng[0].include?('adj')
        system 'say 介词' if eng[0].include?('prep')
        system 'say ' + x[1]
      end
      puts; print "\033[s 直接回车查看下一个单词；输入任意内容以查看网络例句: "
      if $stdin.gets == "\n" then print("\033[1A ----------------------------------------------------------------------------------------\n\n"); return true end
    end
    if @exmp[@wd[0]].empty? then print "\033[u 暂时没有可用或可靠的网络例句. 请按回车键继续."; STDIN.gets end
...
loop do
  begin # Ctrl+C 退出当前
    ...
    while @wn < @hash.size
      ...
      @wd = @list[@wnl[@wn - 1]]
      print " \033[1;4;7;44;37m[#{@wnl[@wn - 1] + 1}]\033[0m "; print @wd[0]; system 'say -v Samantha ' + @wd[0]; print '	'
      r = $stdin.gets
      if r == "\n"
        @known.push(@wnl[@wn - 1]) # 认识单词, 仍然显示、朗读中文释义
        print "\033[1A\033[45C \033[1;7;42;37m已认识 \033[0m\033[1;41;37m ("; @wd[1].each {|x| print x[1] + '; '; system 'say ' + x[1]}
      ...
      begin # Ctrl+C 退出当前
        ...
        if @rec[1..-1].include?(@rn)
          ...
          while @wn < @hash_del[@rn].size
            ...
            @wd = @list[@wnl[@wn - 1]]
            print " \033[1;4;7;44;37m[#{@wnl[@wn - 1] + 1}]\033[0m "; print @wd[0]; system 'say -v Samantha' + @wd[0]; print '	'; r = $stdin.gets.chomp
            if r.empty?
